...
def build_model(input):
    ...
    res2a_relu = build_res_block(input=pool1,block_name='2a',d1=64,d2=256,projection=True)
    ...
    res4f_relu = build_res_block(input=res4e_relu,block_name='4e',d1=256,d2=1024)
...
def build_up_conv_branch(input,branch_name,no_filters,relu = False):
    ...
    conv_D = conv(input=input, name=layerName, stride=1, kernel_size=(2, 2), no_filters=no_filters,
                  use_bias=True)
...
def build_res_block(input,block_name,d1,d2,projection = False):
    # Projection Block
    #                  Upper Branch
    # res[conv(res)-Bn(bn)]2[Box_no]a[Block_char]_branch2[Lower(1)-Upper(2)]a[order @ Branch(a -> b -> c -> d)]
    upper_branch = build_upper_branch(input=input,block_name=block_name,in_depth=d1,out_depth=d2)
    lower_branch = build_lower_branch(input=input,block_name=block_name,out_depth=d2,projection = projection)
    res =  tf.add(upper_branch,lower_branch,name='res' + block_name)
    res_relu = tf.nn.relu(res,name='res2a_relu')
    return res_relu

def build_lower_branch(input,block_name,out_depth,projection=False):
    if projection:
        res_branch1 = conv(input=input,name='res'+block_name+'_branch1',stride=1,kernel_size=(1,1),no_filters=out_depth,use_bias=False)
        bn_branch1 = batch_norm(input=res_branch1,name='bn'+block_name+'_branch1')
        return bn_branch1
    return input

def build_upper_branch(input,block_name,in_depth,out_depth):
    res_branch2a = conv(input=input, name='res'+block_name+'_branch2a', stride=1, kernel_size=(1,1), no_filters=in_depth, use_bias=False)
...
def conv(input,name,stride,kernel_size,no_filters,trainable = False , use_bias = True):
    if use_bias:
